...
function selectInputs(curLine)
    local inputNames = {"Channel ", "1A", "1B", "1C", "2A", "2B", "2C", "3A", "3B", "3C", "4A", "4B", "4C", "5A", "5B",
                        "5C", "6A", "6B", "6C", "7A", "7B", "7C", "8A", "8B", "8C"}
    for k, v in pairs(inputNames) do
        if string.find(curLine, v) then
            if string.find(curLine, "Data2asCV") or string.find(curLine, "XfadeACV") then
                return (false)
            else
                return (true)
            end
...
for k, v in pairs(lines) do
    ...
    if (selectInputs(v)) then
        -- stripping all the spaces from the string
        v = string.gsub(v, "%s+", "")
...
for k, v in pairs(allInputs) do
    ...
    -- composing our resulting string 
